...
- Date: 2026-05-03
- Blueprint source: `cp-program-manager-mcp-integrated-blueprint-2026-05-03.md:673-697`
...
Program Manager is a stateful PMO memory service with its own repository, event ledger, artifact registry, and cp-graph projection store.
...
- Primary implementation package: `cp-program-manager`
- MCP exposure: narrow tool surface via `@mcp-gateway`
...
Neo4j is the primary cp-graph query and projection database, not the only audit history. PMO must be able to rebuild cp-graph projections from PMO events, registry manifests, adapter manifests, artifact metadata, evidence digests, and adapter cursors.
...
- Cross-program dependency analysis has a cp-graph-backed home.
- External tool replacement risk stays low because PMO coordinates rather than absorbs focused systems.

### Costs

- PMO needs explicit repository, cp-graph schema, replay logic, authz, and redaction contracts before implementation.
